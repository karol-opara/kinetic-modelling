function [X_best,Best_fit,All_fit,nfes,res_det]= optimizerAPGSK_IMODE(fhd, D, max_nfes, lBounds, uBounds, varargin)
Par = Introd_Par(D, lBounds, uBounds, max_nfes);
D=Par.n;
lu = [lBounds.'; uBounds.'];
NP=Par.PopSize;
max_NP = NP;
...
Fit = evaluatePopulation(fhd, Pop, varargin{:});
...
while stop_con==0 % nfes<max_nfes
    ...
    if (rand<=Probs(1))
        ...
        [EA_1, EA_1old, EA_obj1,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq, nfes,F,cr, res_det] = ...
            ...
            archive_freq, Par.xmin, Par.xmax,  Par.n,  PS1,  nfes, Par.max_nfes, fhd,res_det,varargin{:});
    ...
    if(rand<Probs(2))
        %% Apply APGSK
        [EA_2,EA_obj2,Par,nfes,res_det]= APGSK_fun(EA_2,EA_obj2,lu,Par,nfes,fhd,res_det, varargin{:});
        if(~isempty(Par.Record_FEs))
            if (nfes>=Par.Record_FEs(1))
...
function [pop,fitness,Par,nfes,res_det]= APGSK_fun(pop,fitness,lu,Par,nfes,fhd,res_det, varargin)
if (size(fitness,2)==1)
    fitness = fitness.';
end
...
children_fitness = evaluatePopulation(fhd, ui, varargin{:});
...
% =========================================================================
function [Par] = Introd_Par(n, lBounds, uBounds, max_nfes)
...
Par.xmin= lBounds.'; %-100*ones(1,Par.n);
Par.xmax= uBounds.'; %100*ones(1,Par.n);
...
function [x, xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq,current_eval,F,cr,res_det ] = ...
    ...
    PopSize,  current_eval, Max_FES,fhd,res_det,varargin)
if (size(fitx,1)==1)
    fitx = fitx.';
end
vi=zeros(PopSize,n);
...
fitx_new = evaluatePopulation(fhd, ui, varargin{:});
%fitx_new = fitx_new.';
...
function [Fit] = evaluatePopulation(fhd, Pop, varargin)
[NP,dim]=size(Pop);
if (dim~=6)
    error('Incorrect dimension');
end
Fit = NaN(1,NP);
for i = 1:NP % one can consider parallel evaluation with parfor i = 1:popSize
    Fit(i) =  feval(fhd, Pop(i,:), varargin{:});
end
Fit = Fit';
end