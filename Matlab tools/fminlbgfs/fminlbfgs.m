function [x,fval,exitflag,output,grad]=fminlbfgs(funfcn,x_init,optim, varargin)
...
%   Updated 2013-07-23 to handle passing additional arguments to the
%   optimized funtion (K. Opara).
...
[data,fval,grad]=gradient_function(data.xInitial,funfcn, data, optim, varargin{:});
data.gradient=grad;
...
while(true)
    ...
    if(optim.GoalsExactAchieve==1)
        data=linesearch(funfcn, data,optim,varargin{:});
    else
        data=linesearch_simple(funfcn, data, optim, varargin{:});
    end
    ...
    if(optim.Display(1)=='p');
        ...
        for i=1:length(alpha_test)
            [data,falpha_test(i)]=gradient_function(data.xInitial(:)+alpha_test(i)*data.dir(:),funfcn, data, optim, varargin{:});
        end
...
function data=linesearch_simple(funfcn, data, optim, varargin)
% Find a bracket of acceptable points
data = bracketingPhase_simple(funfcn, data, optim, varargin{:});
...
if (data.bracket_exitflag  == 2)
    ...
    data = sectioningPhase_simple(funfcn, data, optim, varargin{:});
...
function data = bracketingPhase_simple(funfcn, data,optim, varargin)
...
while(true)
    ...
    if(optim.GradConstr)
        [data,f_alpha]=gradient_function(data.xInitial(:)+alpha*data.dir(:),funfcn, data, optim, varargin{:});
        fPrime_alpha=nan;
        grad=nan;
    else
        [data,f_alpha, grad]=gradient_function(data.xInitial(:)+alpha*data.dir(:),funfcn, data,optim, varargin{:});
    ...
    if(data.beta>0&&hill)
        ...
        if(optim.GradConstr)
            ...
            [data,f_alpha2]=gradient_function(data.xInitial(:)+(alpha+gstep)*data.dir(:),funfcn, data, optim, varargin{:});
            [data,f_beta2]=gradient_function(data.xInitial(:)+(beta+gstep)*data.dir(:),funfcn, data, optim, varargin{:});
...
function data = sectioningPhase_simple(funfcn, data, optim, varargin)
% Get the brackets
...
function data=linesearch(funfcn, data, optim, varargin)

% Find a bracket of acceptable points
data = bracketingPhase(funfcn, data,optim,varargin{:});

if (data.bracket_exitflag  == 2)
    % BracketingPhase found a bracket containing acceptable points;
    % now find acceptable point within bracket
    data = sectioningPhase(funfcn, data, optim,varargin{:});
    data.exitflag = data.section_exitflag;
else
    % Already acceptable point found or MaxFunEvals reached
    data.exitflag = data.bracket_exitflag;
...
function data = sectioningPhase(funfcn, data, optim,varargin)
%
...
function data = bracketingPhase(funfcn, data, optim,varargin)
...
function [data,fval,grad]=gradient_function(x,funfcn, data, optim, varargin)
% Call the error function for error (and gradient)
if ( nargout <3 )
    timem=tic;
    fval=funfcn(reshape(x,data.xsizes),varargin{:});
    data.timeExtern=data.timeExtern+toc(timem);
    data.funcCount=data.funcCount+1;
else
    if(strcmp(optim.GradObj,'on'))
        timem=tic;
        [fval, grad]=feval(funfcn,reshape(x,data.xsizes),varargin{:});
        data.timeExtern=data.timeExtern+toc(timem);
        data.funcCount=data.funcCount+1;
        ...
        for i=1:length(x),
            ...
            timem=tic;
            [fval_g]=feval(funfcn,reshape(x_temp,data.xsizes),varargin{:}); 
            data.funcCount=data.funcCount+1;
            data.timeExtern=data.timeExtern+toc(timem);
