% From https://github.com/elmerzhouhaohao/AMPSO
% https://arxiv.org/abs/2004.07561
function [best_pos, best_fit, out] = optimizerAMPSO(fhd, DIM, lBounds, uBounds, fes_max, varargin)
    global vel_percent;
    global Ner;
    global alpha1;
    global alpha2;
    ...
    swarm = initial(fhd, FUNC, SIZE, DIM, LB, UB, lBounds, uBounds, varargin{:});
    ...
    while Ni < ceil(Ntotal / 3)
        search_swarms = swarm1(search_swarms, N1, lBounds, uBounds, varargin{:});
        Ni = Ni + N1 + 1;
        ...
        [exploit_swarm, step_exploit] = swarm2(exploit_swarm, swarm1_pos, N1, N2, lBounds, uBounds, varargin{:});
    ...
    remain_Ni = Ntotal - Ni;
    convergence_swarm = swarm3(convergence_swarm, swarm2_pos, remain_Ni, lBounds, uBounds, varargin{:});
    ...
    best_pos = decode_population(convergence_swarm.solution.best_pos, lBounds, uBounds);
    out = convergence_swarm.solution;
end


function swarm = initial(fhd, FUNC, SIZE, DIM, LB, UB,lBounds, uBounds, varargin)
    global vel_percent;
    global Ner;
    ...
    for index = 1:N 
        ...
        swarm{1}{index}.particles.fit = evaluate_fitness(swarm{1}{index}.setting.fhd, swarm{1}{index}.particles.pos, lBounds, uBounds, varargin{:});
    ...
    for index = 2:3            
        ...
        swarm{index}.particles.fit = evaluate_fitness(swarm{index}.setting.fhd, swarm{index}.particles.pos, lBounds, uBounds, varargin{:});
...
function swarms_new = swarm1(swarms, N1, lBounds, uBounds, varargin)   
    % reset search_swarm
    N = length(swarms);
    for index = 1:N
        swarm = swarms{index};
        ...
        swarm.particles.fit = evaluate_fitness(swarm.setting.fhd, swarm.particles.pos, lBounds, uBounds, varargin{:});
...
function [swarm_new, step] = swarm2(swarm, original_pos, iter_exploit_min, iter_exploit_max, lBounds, uBounds, varargin)    
    global alpha3
    ...
    while true
        ...
        position = max(swarm.setting.lb, min(swarm.setting.ub, position));
        fitness = evaluate_fitness(swarm.setting.fhd, position, lBounds, uBounds, varargin{:});
        ...
        temp_pos = max(swarm.setting.lb, min(swarm.setting.ub, temp_pos));
        temp_fit = evaluate_fitness(swarm.setting.fhd, temp_pos, lBounds, uBounds, varargin{:});
...
function swarm_new = swarm3(swarm, original_pos, remain_Ni, lBounds, uBounds, varargin)  
    global beta1;
    ...
    while step < remain_Ni
        ...
        if rand < pc
            ...
            position = max(swarm.setting.lb, min(swarm.setting.ub, position));
            swarm.particles.pos = position;
            swarm.particles.fit = evaluate_fitness(swarm.setting.fhd, position, lBounds, uBounds, varargin{:});
            for index = 1:swarm.setting.size
                if swarm.particles.fit(index) < swarm.particles.best_fit(index)
                    swarm.particles.best_pos(index, :) = swarm.particles.pos(index, :);
                    swarm.particles.best_fit(index) = swarm.particles.fit(index);
...
function fitness = evaluate_fitness(fhd, pop, lBounds, uBounds, varargin)
[pop_size, ~] = size(pop);
decode_population(pop, lBounds, uBounds);
fitness = NaN(pop_size, 1);
for i =1:pop_size
    fitness(i) = feval(fhd, pop(i,:), varargin{:});
end
end

function pop2 = decode_population(pop, lBounds, uBounds)
[pop_size, ~] = size(pop);
pop2 = repmat(lBounds.', pop_size, 1) + (repmat(uBounds.', pop_size, 1) - repmat(lBounds.', pop_size, 1)).*pop;
end